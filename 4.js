...
function updateMatrix(){
  ...
  if (total > 0){
    unitPriceEl.textContent = tierPriceFor(product, total) + ' €';
  } else {
  ...
  if (total < 5){
    ...
    warningEl.classList.toggle('show', total > 0);
  } else {
...
document.getElementById('lead-form').addEventListener('submit', e => {
  ...
  const logoInfo = window.isCleanOrder() ? 'Be logotipo (švarios striukės)'
    : (window.hasLogoPlaced() ? 'Su logotipu (' + window.getLogoZone() + ')' : 'Logotipas nenurodytas');
